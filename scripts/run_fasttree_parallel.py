...
def run_fasttree_on_file(
    ...
    if output_tree_path.exists() and output_tree_path.stat().st_size > 0:
        logging.debug(
            f"Output tree {output_tree_path} already exists and is non-empty. Skipping."
        )
        return base_name, "Skipped (Output Exists)"
    elif output_tree_path.exists():
        logging.warning(
            f"Output tree {output_tree_path} exists but is empty. Will overwrite."
        )
    ...
    logging.debug(
        f"Running command for {identifier}: {' '.join(command)} < {aligned_fasta_path} > {output_tree_path}"
    )

    try:
        with (
            open(aligned_fasta_path, "r") as stdin_file,
        ...
        if not output_tree_path.stat().st_size > 0:
            ...
            try:  # Try to read the log file for more info
                with open(output_log_path, "r", encoding="utf-8") as f_log_read:
                    ...
                    if log_content:
                        error_info = f"FastTree Error (see log): {log_content}..."
            except Exception:
                pass
            logging.warning(
                f"FastTree created an empty tree file for {base_name}. Check log: {output_log_path}"
            )
        ...
        logging.error(
            f"FastTree failed for {base_name} with exit code {e.returncode}. Check log: {output_log_path}"
        )
        return base_name, f"Error: FastTree failed (Code {e.returncode})"
    except Exception as e:
        logging.exception(
            f"An unexpected error occurred while running FastTree for {base_name}: {e}"
        )
...
def parse_arguments() -> argparse.Namespace:
    ...
    default_cores = max(
        1, os.cpu_count() - 2 if os.cpu_count() and os.cpu_count() > 2 else 1
    )
    parser.add_argument(
        "-n",
        "--n_jobs",
...
def main():
    ...
    if not fasttree_exe_path:
        logging.error(
            f"FastTree executable '{args.fasttree_exe}' not found in PATH or is not executable."
        )
        sys.exit(1)
    ...
    if not aligned_files:
        logging.error(
            f"No files matching '*{args.input_suffix}' found in {args.input_dir}. Stopping."
        )
        sys.exit(1)

    logging.info(
        f"Found {len(aligned_files)} aligned files to process for tree building."
    )
    ...
    with Parallel(n_jobs=args.n_jobs) as parallel:
        ...
        for i, (fname, status) in enumerate(futures):
            ...
            if processed_count % 50 == 0 or processed_count == len(aligned_files):
                logging.info(
                    f"Processed {processed_count}/{len(aligned_files)} files... (Success: {success_count}, Skipped: {skipped_count}, Errors: {error_count})"
                )
    ...
    logging.info(
        f"Total files processed/skipped: {success_count + skipped_count + error_count}/{len(aligned_files)}"
    )
    ...
    if error_count > 0:
        logging.error(
            f"{error_count} jobs failed. Check logs in {args.log_dir} for details."
        )
        sys.exit(1)
