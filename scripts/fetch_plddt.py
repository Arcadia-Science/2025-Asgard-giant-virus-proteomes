...
# --- Constants ---
AFDB_JSON_URL_TEMPLATE = (
    "https://alphafold.ebi.ac.uk/files/AF-{identifier}-F1-confidence_v4.json"
)
DEFAULT_REQUEST_DELAY = 0.1


def fetch_and_calculate_plddt(
    protein_id_debug: str, identifier: str, afdb_url_template: str
) -> Union[float, None]:
    """
    ...
    if pd.isna(identifier):
        ...
        logging.info(
            f"[{protein_id_debug} | NaN_Identifier]: Failed to retrieve Avg pLDDT. Reason: Identifier was NaN."
        )
        return None
    ...
    if not identifier_str:
        logging.debug(
            f"[{protein_id_debug}]: Identifier is empty after stripping. Skipping."
        )
        logging.info(
            f"[{protein_id_debug} | Empty_Identifier]: Failed to retrieve Avg pLDDT. Reason: Identifier was empty."
        )
        return None

    url = afdb_url_template.format(identifier=identifier_str)
    logging.debug(
        f"[{protein_id_debug} | {identifier_str}]: Processing AFDB ID. Fetching URL: {url}"
    )

    try:
        response = requests.get(url, timeout=30)
        logging.debug(
            f"[{protein_id_debug} | {identifier_str}]: Response Status Code: {response.status_code}"
        )
        ...
        logging.debug(
            f"[{protein_id_debug} | {identifier_str}]: Type of 'data' object: {type(data)}"
        )
        logging.debug(
            f"[{protein_id_debug} | {identifier_str}]: Raw 'data' (first 500 chars): {str(data)[:500]}"
        )
        if isinstance(data, dict):
            logging.debug(
                f"[{protein_id_debug} | {identifier_str}]: Keys in 'data' dict: {list(data.keys())}"
            )
        elif isinstance(data, list) and data:
            logging.debug(
                f"[{protein_id_debug} | {identifier_str}]: 'data' is a list with {len(data)} element(s). Type of 'data[0]': {type(data[0])}"
            )
            if isinstance(data[0], dict):
                logging.debug(
                    f"[{protein_id_debug} | {identifier_str}]: Keys in 'data[0]' dict: {list(data[0].keys())}"
                )
        ...
        if isinstance(data, dict):
            ...
            logging.warning(
                f"[{protein_id_debug} | {identifier_str}]: JSON 'data' object is not a dict or a list of dicts as expected."
            )

        logging.debug(
            f"[{protein_id_debug} | {identifier_str}]: Extracted 'confidence_scores' (type: {type(confidence_scores)}): {str(confidence_scores)[:200] if confidence_scores is not None else 'None'}"
        )

        if (
            confidence_scores
            and isinstance(confidence_scores, list)
            and len(confidence_scores) > 0
        ):
            if all(isinstance(score, (int, float)) for score in confidence_scores):
                avg_confidence = sum(confidence_scores) / len(confidence_scores)
                calculated_avg_plddt = round(avg_confidence, 2)
                logging.debug(
                    f"[{protein_id_debug} | {identifier_str}]: Successfully calculated Avg Confidence Score (pLDDT): {calculated_avg_plddt}"
                )
            else:
                logging.warning(
                    f"[{protein_id_debug} | {identifier_str}]: 'confidenceScore' list contains non-numeric values. Scores: {str(confidence_scores[:10])}"
                )
        else:
            logging.debug(
                f"[{protein_id_debug} | {identifier_str}]: Final check: 'confidenceScore' is None, not a list, or an empty list."
            )

    except requests.exceptions.HTTPError as e:
        if e.response.status_code == 404:
            logging.info(
                f"[{protein_id_debug} | {identifier_str}]: AFDB confidence entry not found (404)."
            )
        else:
            logging.error(
                f"[{protein_id_debug} | {identifier_str}]: HTTP Error {e.response.status_code} - {e.response.text[:200]}."
            )
    except requests.exceptions.RequestException as e:
        logging.error(f"[{protein_id_debug} | {identifier_str}]: Request failed: {e}.")
    except json.JSONDecodeError as e_json:
        logging.error(
            f"[{protein_id_debug} | {identifier_str}]: Could not decode JSON. Error: {e_json}."
        )
    except Exception as e:
        logging.error(
            f"[{protein_id_debug} | {identifier_str}]: An unexpected error occurred: {e}."
        )
        logging.exception("Traceback for unexpected error:")

    if calculated_avg_plddt is not None:
        logging.info(
            f"[{protein_id_debug} | {identifier_str}]: Avg Confidence Score (pLDDT) {calculated_avg_plddt} successfully retrieved from {url}"
        )
    else:
        logging.info(
            f"[{protein_id_debug} | {identifier_str}]: Failed to retrieve Avg Confidence Score (pLDDT) from {url}."
        )
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--afdb_url_template",
        default=AFDB_JSON_URL_TEMPLATE,
...
def main():
    ...
    logging.info("Starting pLDDT fetch process...")
    ...
    try:
        ...
        if args.protein_id_col not in df_input.columns:
            logging.error(
                f"Error: Column '{args.protein_id_col}' not found in {args.input_csv}. Please check configuration."
            )
            sys.exit(1)
        if args.afdb_id_col not in df_input.columns:
            logging.error(
                f"Error: Column '{args.afdb_id_col}' not found in {args.input_csv}. Please check configuration."
            )
            sys.exit(1)
        ...
        if args.limit is not None and args.limit > 0:
            logging.info(
                f"Processing only the first {args.limit} rows due to --limit option."
            )
        ...
        for index, row in iterable_rows:
            ...
            avg_plddt = fetch_and_calculate_plddt(
                str(protein_id), afdb_id_original, args.afdb_url_template
            )
