...
def load_processed_ids(filename: Path) -> set[str]:
    ...
    if filename.exists() and filename.stat().st_size > 0:  # Check size > 0
        logging.info(
            f"Found existing results file: {filename}. Loading processed IDs..."
        )
        try:
            with open(filename, "r", newline="", encoding="utf-8") as f:
                ...
                try:
                    ...
                    if header != ["UniProtKB_AC", "PDB_IDs"]:
                        logging.warning(
                            f"Output file header mismatch. Expected ['UniProtKB_AC', 'PDB_IDs'], got {header}. Attempting to read anyway."
                        )
                except StopIteration:
                    logging.warning(
                        f"Output file {filename} exists but is empty or has no header. Starting fresh."
                    )
            ...
            logging.warning(
                f"Error reading existing results file {filename}: {e}. Assuming no IDs processed."
            )
...
def query_uniprot_single(
    accession: str, max_retries: int, retry_delay: int
) -> tuple[str, str]:
    """
    ...
    while retries <= max_retries:
        try:
            ...
            if response.status_code == 400:
                logging.warning(
                    f"  Received 400 Bad Request for {accession}. Invalid AC? Skipping."
                )
                return (accession, "Error: 400 Bad Request")
            if response.status_code == 404:
                logging.info(
                    f"  Received 404 Not Found for {accession} (Accession invalid or not found)."
                )
                return (accession, "Error: 404 Accession Not Found")
            if response.status_code == 410:
                logging.info(
                    f"  Received 410 Gone for {accession} (Accession obsolete)."
                )
            ...
            try:
                ...
                try:
                    ...
                    if len(data_line) >= 2:  # Should have at least Entry and PDB field
                        ...
                        logging.debug(
                            f"  Data line only contained entry for {accession}, assuming no PDB refs."
                        )
                        return (accession, "")  # Return empty string for PDB IDs
                    else:  # Unexpected data format
                        logging.warning(
                            f"Unexpected data format (empty line?) for {accession}"
                        )
                    ...
                    logging.debug(
                        f"  No data line found for {accession} after header. No PDB refs."
                    )
                ...
                logging.warning(
                    f"Empty response received despite 200 OK for {accession}"
                )
            ...
            retries += 1
            logging.warning(
                f"  Request timed out for {accession} (Attempt {retries}/{max_retries + 1}). Retrying in {retry_delay}s..."
            )
            if retries <= max_retries:
                time.sleep(retry_delay)
        except (
            requests.exceptions.RequestException
        ) as e:  # Includes HTTPError for 5xx etc.
            retries += 1
            logging.warning(
                f"  Request failed for {accession} (Attempt {retries}/{max_retries + 1}): {e}. Retrying in {retry_delay}s..."
            )
            if retries <= max_retries:
                time.sleep(retry_delay)
        except Exception as e:
            logging.error(
                f"  Unexpected error during UniProt query for {accession}: {e}"
            )
...
def save_single_result(
    result_tuple: tuple[str, str] | None, filename: Path, write_header: bool
):
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--max_retries",
        type=int,
        default=2,  # Increased default slightly
        help="Max retries for a failed request (timeouts, server errors).",
    )
    parser.add_argument(
        "--retry_delay", type=int, default=5, help="Delay between retries in seconds."
    )
    parser.add_argument(
        "--progress", type=int, default=100, help="Print progress update every N IDs."
    )
...
if __name__ == "__main__":
    ...
    try:
        ...
        logging.critical(
            "The 'requests' library is required. Please install it (`pip install requests`)."
        )
        sys.exit(1)
    ...
    try:
        ...
        ids_to_query_now = [
            pid for pid in all_target_ids if pid not in processed_ids_set
        ]
        total_remaining = len(ids_to_query_now)

        if not ids_to_query_now:
            logging.info(
                "All UniProt IDs from the input file have already been processed according to the results file."
            )
            sys.exit(0)

        logging.info(
            f"Starting query for {total_remaining:,} remaining UniProt IDs (one by one using GET)..."
        )
        # Determine if header needs to be written
        is_new_file = (
            not args.output_file.exists() or args.output_file.stat().st_size == 0
        )
        ...
        for i, current_id in enumerate(ids_to_query_now):
            ...
            result = query_uniprot_single(
                current_id, args.max_retries, args.retry_delay
            )
            ...
            if result:
                ...
                if "Error:" in res_string:
                    ...
                    logging.info(
                        f"  Result for {acc}: Found PDB: {res_string[:100]}{'...' if len(res_string) > 100 else ''}"
                    )
                ...
                logging.error(
                    f"  No result tuple obtained for {current_id}. This indicates an internal script error."
                )
                session_error_count += 1
        ...
        # Final summary
        final_processed_in_file = len(
            load_processed_ids(args.output_file)
        )  # Re-check file
        logging.info("\n--- Processing Complete ---")
        logging.info(
            f"Attempted queries for {session_processed_count} IDs in this session."
        )
        logging.info(
            f"Total unique IDs processed and saved in output file: {final_processed_in_file}/{len(all_target_ids)}"
        )
        logging.info(
            f"Total result lines saved/appended in this session: {session_saved_count}"
        )
