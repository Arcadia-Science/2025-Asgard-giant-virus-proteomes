...
def run_mafft_on_file(
    ...
    if output_aln_path.exists() and output_aln_path.stat().st_size > 0:
        logging.debug(
            f"Output alignment {output_aln_path} already exists and is non-empty. Skipping."
        )
        return base_name, "Skipped (Output Exists)"
    elif output_aln_path.exists():
        logging.warning(
            f"Output alignment {output_aln_path} exists but is empty. Will overwrite."
        )
    ...
    try:
        ...
        if not output_aln_path.stat().st_size > 0:
            ...
            try:  # Try to read the log file for more info
                ...
                pass  # Ignore errors reading the log file itself
            logging.warning(
                f"MAFFT created an empty alignment file for {base_name}. Check log: {output_log_path}"
            )
        ...
        logging.error(
            f"MAFFT failed for {base_name} with exit code {e.returncode}. Check log: {output_log_path}"
        )
        return base_name, f"Error: MAFFT failed (Code {e.returncode})"
    except Exception as e:
        # Catch any other unexpected errors during subprocess execution
        logging.exception(
            f"An unexpected error occurred while running MAFFT for {base_name}: {e}"
        )
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--mafft_exe", default="mafft", help="Path to the MAFFT executable."
    )
    parser.add_argument(
        "--mafft_args",
        default="--auto --thread 1",
    ...
    default_cores = max(
        1, os.cpu_count() - 1 if os.cpu_count() and os.cpu_count() > 1 else 1
    )
    parser.add_argument(
        "-n",
        "--num_cores",
...
def main():
    ...
    if not mafft_exe_path:
        logging.error(
            f"MAFFT executable '{args.mafft_exe}' not found in PATH or is not executable."
        )
        sys.exit(1)
    ...
    with ProcessPoolExecutor(max_workers=args.num_cores) as executor:
        ...
        for i, future in enumerate(as_completed(futures)):
            ...
            try:
                ...
                logging.error(
                    f"Job for {input_fname} generated an exception in the executor: {exc}"
                )
                results.append(
                    {
                        "file": input_fname,
                        "status": f"Error: Future Exception ({type(exc).__name__})",
                    }
                )
                error_count += 1
            ...
            if processed_count % 50 == 0 or processed_count == len(fasta_files):
                logging.info(
                    f"Processed {processed_count}/{len(fasta_files)} files... (Success: {success_count}, Skipped: {skipped_count}, Errors: {error_count})"
                )
    ...
    logging.info(
        f"Total files processed/skipped: {success_count + skipped_count + error_count}/{len(fasta_files)}"
    )
