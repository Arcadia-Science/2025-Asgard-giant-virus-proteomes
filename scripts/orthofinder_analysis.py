...
def parse_fasta_header(record, default_phylum="UnknownPhylum"):
    ...
    *** EDIT THIS FUNCTION *** if your FASTA header format differs significantly
    ...
    """
    protein_id = "UnknownID"
    genome_id = "UnknownGenome"
    ...
    try:
        ...
        if len(parts) >= 5:  # >ProteinID|GenomeID|Phylum|AnnotationType|CleanedName
            ...
            cleaned_annotation = (
                parts[4] if parts[4] and annotation_type == "annotated" else None
            )
            ...
            len(parts) >= 4
        ):  # >ProteinID|GenomeID|Phylum|AnnotationType (Name missing)
            protein_id = parts[0]
            genome_id = parts[1]
            phylum = parts[2] if parts[2] else default_phylum
            annotation_type = parts[3].lower() if parts[3] else "hypothetical"
            if annotation_type == "annotated":
                ...
                cleaned_annotation = clean_annotation_name(
                    desc_only
                )  # Use helper for cleaning
                if cleaned_annotation is None:
                    annotation_type = "hypothetical"  # Revert if parsing fails
            else:
                cleaned_annotation = None
                if annotation_type not in ["annotated", "hypothetical"]:
                    annotation_type = "hypothetical"
        ...
        logging.warning(
            f"Could not parse header for record starting with '{record.id[:50]}...': {e}"
        )
        ...
        return {
            ...
            "GenomeID": "ParseError",
            "Phylum": default_phylum,
            "AnnotationType": "hypothetical",
            "CleanedAnnotationName": None,
        }
...
def clean_annotation_name(annotation_string):
    ...
    if re.match(generic_terms, cleaned, flags=re.IGNORECASE):
        return None
    return cleaned
...
def load_phylum_map(map_file):
    ...
    try:
        ...
        if accession_col and phylum_col:
            phylum_map = (
                phylum_df_map.set_index(accession_col)[phylum_col].astype(str).to_dict()
            )
            logging.info(f"Loaded phylum information for {len(phylum_map)} accessions.")
        else:
            logging.error(
                f"Phylum map file '{map_file}' missing required columns (need ~'accession' and ~'phylum'). Found: {list(phylum_df_map.columns)}"
            )
...
def load_protein_metadata(fasta_dir, phylum_map=None):
    ...
    for f_path in fasta_files:
        ...
        genome_id_from_fname = (
            f_path.stem
        )  # e.g., GCA_0012345.1 from GCA_0012345.1.fasta

        try:
            for record in SeqIO.parse(f_path, "fasta"):
                parsed_proteins_count += 1
                metadata = parse_fasta_header(
                    record
                )  # Get initial metadata from header
                ...
                if metadata:
                    ...
                    if phylum_map and metadata["GenomeID"] in phylum_map:
                        ...
                        metadata["Phylum"] = phylum_map.get(
                            metadata["GenomeID"], "UnknownPhylum"
                        )

                    protein_metadata.append(metadata)

            if processed_files_count % 50 == 0 or processed_files_count == len(
                fasta_files
            ):
                logging.info(
                    f"  ...parsed {processed_files_count}/{len(fasta_files)} files ({parsed_proteins_count:,} proteins)..."
                )
    ...
    if not protein_metadata:
        logging.error(
            "Failed to load any protein metadata. Check FASTA headers and parsing function."
        )
        return None

    protein_meta_df = pd.DataFrame(protein_metadata)
    logging.info(
        f"Loaded metadata for {len(protein_meta_df):,} proteins from {processed_files_count} files."
    )
    ...
    if protein_meta_df["ProteinID"].duplicated().any():
        ...
        logging.warning(
            f"{duplicate_count} duplicate ProteinIDs found in metadata. Using first occurrence. Ensure ProteinIDs are unique across all input FASTAs if this is unexpected."
        )
        protein_meta_df = protein_meta_df.drop_duplicates(
            subset=["ProteinID"], keep="first"
        )
        logging.info(
            f"  DataFrame reduced to {len(protein_meta_df)} rows after dropping duplicate ProteinIDs."
        )
...
def calculate_og_stats(og_df_wide):
    ...
    try:
        ...
        og_df_long_with_str["ProteinID_list"] = og_df_long_with_str[
            "ProteinString"
        ].str.split(", ")

        logging.debug("Exploding dataframe...")
        og_df_long = og_df_long_with_str.explode("ProteinID_list").drop(
            columns=["ProteinString"]
        )
        ...
        logging.info(
            f"Reshaped orthogroup data into {len(og_df_long):,} protein assignments."
        )
    ...
    try:
        og_stats = (
            og_df_long.groupby("Orthogroup")
            .agg(
                num_proteins=("ProteinID", "size"), num_species=("GenomeID", "nunique")
            )
            .reset_index()
        )
...
def merge_and_annotate(og_df_long, protein_meta_df, phylum_col_name):
    ...
    try:
        ...
        logging.error(
            f"Merge failed. Missing key column: {e}. Check ProteinID column name in both datasets."
        )
        return None, None
    except Exception as e:
    ...
    if missing_meta_count > 0:
        logging.warning(
            f"{missing_meta_count:,} protein assignments lack metadata after merge."
        )
        missing_ids = merged_data.loc[missing_meta_mask, "ProteinID"].unique()
        logging.warning(
            f"  Example ProteinIDs missing metadata: {list(missing_ids[:10])}"
        )
        logging.warning(
            "  This might happen if FASTA files used for OrthoFinder differ from those used for metadata extraction, or due to header parsing issues."
        )
    ...
    try:
        og_annot_summary = (
            merged_data.groupby("Orthogroup")["AnnotationType"]
            .value_counts()
            .unstack(fill_value=0)
        )
        if "hypothetical" not in og_annot_summary.columns:
            og_annot_summary["hypothetical"] = 0
        if "annotated" not in og_annot_summary.columns:
            og_annot_summary["annotated"] = 0
        og_annot_summary["total_annot_types"] = (
            og_annot_summary["hypothetical"] + og_annot_summary["annotated"]
        )
        ...
        og_annot_summary.loc[valid_totals, "pct_hypothetical"] = (
            og_annot_summary["hypothetical"][valid_totals]
            / og_annot_summary["total_annot_types"][valid_totals]
        ) * 100
    ...
    if phylum_col_name in merged_data.columns:
        ...
        try:
            ...
            og_phylum_counts = merged_data.groupby("Orthogroup")[
                phylum_col_name
            ].nunique()
            og_phylum_stats = og_phylum_counts.reset_index(
                name=f"num_{phylum_col_name.lower()}"
            )
    ...
    if og_phylum_stats is not None:
        og_summary_stats = pd.merge(
            og_summary_stats, og_phylum_stats, on="Orthogroup", how="left"
        )

    return merged_data, og_summary_stats


def filter_and_save_ogs(
    og_stats_full,
    merged_data,
    num_species,
    conservation_threshold_pct,
    hypothetical_threshold_pct,
    annotated_threshold_pct,
    output_prefix,
    phylum_col_name,
):
    """Filters OGs based on conservation and annotation status, saves lists including phylum if available."""
    logging.info(
        f"Identifying and saving conserved OGs (>= {conservation_threshold_pct}% species)..."
    )

    conservation_threshold_count = int(
        np.ceil(num_species * (conservation_threshold_pct / 100.0))
    )
    if conservation_threshold_pct > 0 and conservation_threshold_count == 0:
        conservation_threshold_count = 1
    logging.info(
        f"  Absolute species count threshold for conservation: {conservation_threshold_count}/{num_species}"
    )

    conserved_ogs = og_stats_full[
        og_stats_full["num_species"] >= conservation_threshold_count
    ].copy()
    logging.info(
        f"Found {len(conserved_ogs):,} OGs conserved at >= {conservation_threshold_pct}% level."
    )

    if conserved_ogs.empty:
        logging.warning(
            "No OGs met the conservation threshold. Skipping detailed list saving."
        )
        return
    ...
    protein_output_cols = [
        ...
        "ProteinID",
        "AnnotationType",
        "CleanedAnnotationName",
    ]
    if phylum_col_name in conserved_ogs.columns:
        og_output_cols.append(
            f"num_{phylum_col_name.lower()}"
        )  # Add phylum count column if present
    if phylum_col_name in merged_data.columns:
        protein_output_cols.insert(
            3, phylum_col_name
        )  # Insert phylum before annotation type

    # --- Conserved Hypothetical ---
    hypo_pct_filter = hypothetical_threshold_pct
    logging.info(
        f"Filtering for Conserved Hypothetical (>= {hypo_pct_filter}% hypothetical)..."
    )
    conserved_hypothetical_ogs = conserved_ogs[
        conserved_ogs["pct_hypothetical"] >= hypo_pct_filter
    ]
    logging.info(
        f"  Found {len(conserved_hypothetical_ogs):,} conserved hypothetical orthogroups."
    )

    if not conserved_hypothetical_ogs.empty:
        conserved_hypothetical_ogs_sorted = conserved_hypothetical_ogs.sort_values(
            "num_species", ascending=False
        )
        ...
        try:
            conserved_hypothetical_ogs_sorted[og_output_cols].to_csv(
                hypo_ogs_file, sep="\t", index=False, float_format="%.1f"
            )
            logging.info(f"  Saved conserved hypothetical OG stats to: {hypo_ogs_file}")
            conserved_hypo_proteins = merged_data[
                merged_data["Orthogroup"].isin(conserved_hypothetical_ogs["Orthogroup"])
            ]
            conserved_hypo_proteins[protein_output_cols].to_csv(
                hypo_proteins_file, sep="\t", index=False
            )
            logging.info(
                f"  Saved conserved hypothetical protein list ({len(conserved_hypo_proteins):,} proteins) to: {hypo_proteins_file}"
            )
        except IOError as e:
            logging.error(f"  Failed to write conserved hypothetical output files: {e}")
        except KeyError as e:
            logging.error(
                f"  Missing column for saving conserved hypothetical data: {e}. Columns available: {list(conserved_hypothetical_ogs_sorted.columns)} / {list(conserved_hypo_proteins.columns)}"
            )
    else:
    ...
    logging.info(
        f"Filtering for Conserved Annotated (>= {annotated_threshold_pct}% annotated, i.e., <= {anno_pct_filter:.1f}% hypothetical)..."
    )
    conserved_annotated_ogs = conserved_ogs[
        conserved_ogs["pct_hypothetical"] <= anno_pct_filter
    ]
    logging.info(
        f"  Found {len(conserved_annotated_ogs):,} conserved annotated orthogroups."
    )

    if not conserved_annotated_ogs.empty:
        conserved_annotated_ogs_sorted = conserved_annotated_ogs.sort_values(
            "num_species", ascending=False
        )
        ...
        try:
            ...
            logging.info(
                conserved_annotated_ogs_sorted[og_output_cols]
                .round(1)
                .head()
                .to_string()
            )
            conserved_annotated_ogs_sorted[og_output_cols].to_csv(
                anno_ogs_file, sep="\t", index=False, float_format="%.1f"
            )
            logging.info(f"   Saved OG stats to: {anno_ogs_file}")
            conserved_anno_proteins = merged_data[
                merged_data["Orthogroup"].isin(conserved_annotated_ogs["Orthogroup"])
            ]
            conserved_anno_proteins[protein_output_cols].to_csv(
                anno_proteins_file, sep="\t", index=False
            )
            logging.info(
                f"   Saved conserved annotated protein list ({len(conserved_anno_proteins):,} proteins) to: {anno_proteins_file}"
            )
        except IOError as e:
            logging.error(f"  Failed to write conserved annotated output files: {e}")
        except KeyError as e:
            logging.error(
                f"  Missing column for saving conserved annotated data: {e}. Columns available: {list(conserved_annotated_ogs_sorted.columns)} / {list(conserved_anno_proteins.columns)}"
            )
    else:
...
def write_summary_stats_file(
    ...
    try:
        with open(output_file, "w") as f:
            f.write("OrthoFinder Summary Statistics\n")
            ...
            if phylum_col_name in merged_data.columns:
                ...
                try:
                    ...
                    og_phylum_presence = merged_data.drop_duplicates(
                        subset=["Orthogroup", phylum_col_name]
                    )[phylum_col_name].value_counts()
                    f.write(
                        f"Number of OGs Containing at least one Protein from {phylum_col_name}:\n"
                    )
                    f.write(og_phylum_presence.to_string())
                    f.write("\n\n")
                except Exception as phylum_e:
                    logging.warning(
                        f"Could not calculate or write phylum summary statistics: {phylum_e}"
                    )
...
def generate_plots(
    ...
    try:
        ...
        plt.title(
            f"{output_prefix}: Distribution of Species per OG (Total OGs = {num_orthogroups:,})"
        )
        plt.tight_layout()
        if plot_output_dir:
            plot_path = os.path.join(
                plot_output_dir, f"{output_prefix}_species_per_og_dist.png"
            )
        ...
        logging.error(f"Error generating species distribution plot: {e}")
    ...
    try:
        ...
        if not plot_data.empty:
            ...
            plt.ylabel("Number of Orthogroups")
            plt.title(
                f"{output_prefix}: Distribution of Proteins per OG (Total OGs = {num_orthogroups:,})"
            )
            plt.tight_layout()
            if plot_output_dir:
                plot_path = os.path.join(
                    plot_output_dir, f"{output_prefix}_proteins_per_og_dist.png"
                )
                plt.savefig(plot_path, dpi=300)
            ...
            logging.warning("Skipping protein distribution plot (no data).")
    except Exception as e:
        logging.error(f"Error generating protein distribution plot: {e}")

    # Plot 3: Core OG Counts
    if core_og_counts:
        try:
            # Convert dict keys (like '>= 90%') to numeric for sorting if needed
            core_og_counts_df = pd.DataFrame(
                list(core_og_counts.items()), columns=["Threshold_Label", "Count"]
            )
            core_og_counts_df["Threshold_Num"] = (
                core_og_counts_df["Threshold_Label"]
                .str.extract(r"(\d+\.?\d*)")
                .astype(float)
            )
            core_og_counts_df = core_og_counts_df.sort_values(
                by="Threshold_Num", ascending=False
            )
            ...
            sns.barplot(
                x="Threshold_Label",
                y="Count",
                data=core_og_counts_df,
                color="skyblue",
                order=core_og_counts_df["Threshold_Label"],
            )
            plt.xlabel("Minimum Conservation Threshold (% Species)")
            plt.ylabel("Number of Orthogroups")
            plt.title(
                f"{output_prefix}: Number of Orthogroups vs. Conservation Threshold"
            )
            plt.xticks(rotation=45, ha="right")
            plt.tight_layout()
            if plot_output_dir:
                plot_path = os.path.join(
                    plot_output_dir, f"{output_prefix}_core_og_counts_by_threshold.png"
                )
                plt.savefig(plot_path, dpi=300)
            ...
            logging.error(f"Error generating core OG count plot: {e}")
    else:
        logging.warning("Skipping core OG count plot (no data calculated).")


# --- Main Execution ---
def main():
    parser = argparse.ArgumentParser(
        description="Analyze OrthoFinder results, integrate with protein metadata, and optionally include phylum analysis."
    )
    ...
    parser.add_argument(
        ...
        "--orthofinder_dir",
        required=True,
        help="Path to OrthoFinder results directory.",
    )
    parser.add_argument(
        "-f",
        "--fasta_dir",
        required=True,
        help="Path to directory containing input FASTA files used for OrthoFinder.",
    )
    parser.add_argument(
        "--phylum_map",
        default=None,
        help="Optional path to CSV/TSV file mapping GenomeID (col 'accession') to Phylum (col 'updated_phylum').",
    )
    ...
    parser.add_argument(
        ...
        "--output_prefix",
        required=True,
        help="Prefix for output files (e.g., 'GV_Analysis').",
    )
    parser.add_argument(
        "--plot_dir", default=None, help="Directory to save plots (optional)."
    )
    parser.add_argument(
        "--summary_file",
        default=None,
        help="File path for summary statistics text (optional, uses prefix otherwise).",
    )
    parser.add_argument(
        "--phylum_column_name",
        default="Phylum",
        help="Name to use for the phylum column in outputs (default: Phylum).",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=80.0,
        help="Conservation threshold (%% species) for saving detailed OG lists (default: 80.0).",
    )
    parser.add_argument(
        "--hypothetical_threshold",
        type=float,
        default=75.0,
        help="Threshold (%%) for 'mostly hypothetical' OGs (default: 75.0).",
    )
    parser.add_argument(
        "--annotated_threshold",
        type=float,
        default=75.0,
        help="Threshold (%%) for 'mostly annotated' OGs (default: 75.0).",
    )
    parser.add_argument(
        "--core_pcts",
        nargs="+",
        type=float,
        default=[100, 99, 95, 90, 80, 70, 50],
        help="Percentages for core OG counts (default: 100 99 95 90 80 70 50).",
    )
    ...
    if args.plot_dir:
        try:
            ...
            logging.info(f"Plots will be saved in: {args.plot_dir}")
        except OSError as e:
            logging.warning(
                f"Could not create plot directory {args.plot_dir}: {e}. Plots will only be displayed."
            )
            args.plot_dir = None
    ...
    if args.phylum_map:
        ...
        if phylum_map_data is None:
            logging.warning(
                "Phylum map loading failed. Proceeding without phylum analysis."
            )

    og_df_wide = load_orthogroups(args.orthofinder_dir)
    if og_df_wide is None:
        sys.exit(1)
    num_orthogroups_loaded = len(og_df_wide)

    protein_meta_df = load_protein_metadata(args.fasta_dir, phylum_map_data)
    if protein_meta_df is None:
        sys.exit(1)

    og_stats, og_df_long, num_species = calculate_og_stats(og_df_wide)
    if og_stats is None or og_df_long is None:
        sys.exit(1)

    # Pass the desired phylum column name for consistency
    merged_data, og_summary_stats = merge_and_annotate(
        og_df_long, protein_meta_df, args.phylum_column_name
    )
    if merged_data is None or og_summary_stats is None:
        sys.exit(1)
    ...
    if phylum_count_col in og_stats_full.columns:
        og_stats_full[phylum_count_col] = (
            og_stats_full[phylum_count_col].fillna(0).astype(int)
        )
    ...
    core_og_counts = write_summary_stats_file(
        ...
        merged_data,
        num_species,
        num_orthogroups_loaded,
        args.core_pcts,
        args.summary_file,
        args.orthofinder_dir,
        args.phylum_column_name,
    )
    ...
    filter_and_save_ogs(
        ...
        args.hypothetical_threshold,
        args.annotated_threshold,
        args.output_prefix,
        args.phylum_column_name,
    )
