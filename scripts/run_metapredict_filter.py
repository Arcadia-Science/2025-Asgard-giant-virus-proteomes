...
try:
    ...
    print(
        "ERROR: Metapredict library not found. Please install it (`pip install metapredict`).",
        file=sys.stderr,
    )
    sys.exit(1)
...
try:
    ...
    print(
        "ERROR: Biopython library not found. Please install it (`pip install biopython`).",
        file=sys.stderr,
    )
    sys.exit(1)
...
def process_fasta_file(
    input_filepath, globular_dir, skipped_writer, disordered_writer, threshold, log
):
    """
    ...
    try:
        for record in SeqIO.parse(input_filepath, "fasta"):
            ...
            if not record.seq:
                log(
                    "WARN",
                    f"Skipping record '{record.id}' in '{basename}' due to empty sequence.",
                )
                file_error_other += 1
                continue
            ...
            if not sequence:  # Check again after potential conversion issues
                log(
                    "WARN",
                    f"Skipping record '{record.id}' in '{basename}' (empty after conversion).",
                )
                file_error_other += 1
                continue
            ...
            try:
                ...
                log(
                    "ERROR",
                    f"Metapredict failed for record '{record.id}' in '{basename}': {predict_err}",
                )
            ...
            if disorder_scores is None:
                log(
                    "WARN",
                    f"Metapredict returned None for record '{record.id}' in '{basename}'. Skipping.",
                )
                file_error_predict += 1
                continue
            try:
                ...
                if np.isnan(average_disorder):
                    log(
                        "WARN",
                        f"Metapredict average score is NaN for record '{record.id}' in '{basename}'. Skipping.",
                    )
                    file_error_predict += 1
                    continue
                ...
                log(
                    "ERROR",
                    f"Calculating mean disorder for '{record.id}' in '{basename}': {mean_err}",
                )
                file_error_other += 1
                continue
        ...
        if globular_records_to_write:
            try:
                ...
                log(
                    "ERROR",
                    f"Failed to write globular output file '{globular_output_filepath}': {write_err}",
                )
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Predict protein disorder and filter FASTA files."
    )
    parser.add_argument(
        "-i",
        "--input-dir",
...
def main():
    ...
    input_fasta_files = glob.glob(os.path.join(args.input_dir, "*.fasta")) + glob.glob(
        os.path.join(args.input_dir, "*.faa")
    )  # Add .faa common extension
    ...
    try:
        with (
            open(args.skipped_file, "w") as skipped_fh,
            ...
            for i, input_filepath in enumerate(sorted(input_fasta_files)):
                ...
                print(
                    f"  Processing file {i + 1}/{len(input_fasta_files)}: {basename}...",
                    end="\r",
                    flush=True,
                )
                ...
                try:
                    ...
                    log(
                        "ERROR",
                        f"FATAL unhandled exception during processing of {basename}: {proc_err}",
                    )
        ...
        log(
            "ERROR", f"Could not open summary output file {e.filename} for writing: {e}"
        )
    ...
    log(
        "INFO",
        f"Processed {total_files_processed}/{len(input_fasta_files)} input files in {elapsed_time:.2f} seconds.",
    )
    if failed_files:
        log(
            "WARN",
            f"Processing failed for {len(failed_files)} files: {', '.join(failed_files)}",
        )
    ...
    log(
        "INFO",
        f"Total predicted disordered (score >= {args.threshold}, no 'X'): {total_discarded_disordered:,}",
    )
    if total_discarded_disordered > 0:
        log("INFO", f"Predicted disordered sequences saved to: {args.disordered_file}")
    log(
        "INFO",
        f"Total predicted globular (score < {args.threshold}, no 'X'): {total_records_kept_globular:,}",
    )
    log("INFO", f"Total Metapredict errors encountered: {total_prediction_errors:,}")
    log(
        "INFO",
        f"Total other processing errors (e.g., empty seq): {total_other_errors:,}",
    )
    ...
    if valid_for_prediction > 0:
        ...
        log(
            "INFO",
            f"Of {valid_for_prediction:,} proteins submitted for prediction (no 'X', no errors):",
        )
        log(
            "INFO",
            f"  - Kept as globular: {total_records_kept_globular:,} ({kept_percentage:.1f}%)",
        )
        log(
            "INFO",
            f"  - Discarded as disordered: {total_discarded_disordered:,} ({discarded_percentage:.1f}%)",
        )
    elif total_records_processed > 0:
        log(
            "WARN",
            "No valid proteins were successfully submitted for prediction (check for 'X' or errors).",
        )
    else:
    ...
    print(
        f"Run OrthoFinder using '{args.globular_dir}' as the input directory ('-f' option)."
    )

    if total_prediction_errors > 0 or total_other_errors > 0 or failed_files:
        log(
            "WARN",
            "Script finished with some errors or warnings. Please review the log.",
        )
